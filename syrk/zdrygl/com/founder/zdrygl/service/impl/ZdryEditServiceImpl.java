...
public class ZdryEditServiceImpl extends BaseService implements ZdryEditService {
    ...
    public List<ZdryxxzsVO> queryRyzsxx(Map<String, Object> map) {
        List<Map<String, String>> list = new Gson().fromJson((String) map.get("zdrylxJson"), new TypeToken<List<Map<String, String>>>() {
        }.getType());
        ...
        if ("ZDRK".equals(list.get(0).get("fz"))) {
            ZdryZdryzb zdryZdryzb = (ZdryZdryzb) zdryZdryzbDao.queryById(list.get(0).get("zdryid"));
            if (zdryZdryzb != null && "020501".equals(zdryZdryzb.getZdrylb())) {
                map.remove("dlbh");
                map.put("isEdit", "1");
                map.put("zdrylx", "XDRY");//吸毒人员
                map.remove("gxzrq");
                map.put("xxdxlxdm", "1");
                temp = zdryEditDao.queryRyzsxx(map);
                if (temp != null && !temp.isEmpty()) {
    ...
    @Override
    public List zdryxdrynjxxb_query(Map<String, Object> map) {
        map.put("maxNum", "10");
        map.put("zdryid", map.get("zdryid"));
        return zdryXdrynjxxbDao.queryViewList(map);
    }

    @Override
    public List zdryxdryxdqkxxb_query(Map<String, Object> map) {
        map.put("maxNum", "10");
        map.put("zdryid", map.get("zdryid"));
        return zdryXdryxdqkxxbDao.queryViewList(map);
    }
